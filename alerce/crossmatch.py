...
class AlerceXmatch(Client):
    ...
    def _request_catshtm(
        self, method, url, params=None, result_format="json"
    ):
    ...
    def _format_all(self, catalog_list, result_format="json"):
        ...
        for idx, r in enumerate(catalog_list):
          ...
          for field in r[key].keys():
              data = r[key][field]['values'] if 'values' in r[key][field] else [r[key][field]['value']]
              t.add_column(Column(data, name=field))
              t[field].unit = r[key][field]['units'] if 'units' in r[key][field] else r[key][field]['unit']
    ...
    def _format_one(self, catalog, result_format="json"):
        ...
        for field in catalog_data.keys():
          data = catalog_data[field]['values'] if 'values' in catalog_data[field] else [catalog_data[field]['value']]
          t.add_column(Column(data, name=field))
          t[field].unit = catalog_data[field]['units'] if 'units' in catalog_data[field] else catalog_data[field]['unit']
    ...
    def catshtm_conesearch(self, ra, dec, radius, catalog_name="all", format="pandas"):
        ...
        if params["catalog"] == "all":
            q = self._request_catshtm("GET",
                            url=self.config["CATSHTM_API_URL"] + self.config["CATSHTM_ROUTES"]["conesearch_all"],
                            result_format=format,
                            params=params)
            q = self._format_all(q["catalogs"], result_format=format)
        else:
            q = self._request_catshtm("GET",
                           url=self.config["CATSHTM_API_URL"] + self.config["CATSHTM_ROUTES"]["conesearch"],
                           result_format=format,
                           params=params)
    ...
    def catshtm_crossmatch(self, ra, dec, radius, catalog_name="all", format="pandas"):
        ...
        if catalog_name == "all":
            q = self._request_catshtm("GET",
                            url=self.config["CATSHTM_API_URL"] + self.config["CATSHTM_ROUTES"]["crossmatch_all"],
                            result_format=format,
                            params=params)
            q = self._format_all(q, result_format=format)
        else:
            q = self._request_catshtm("GET",
                           url=self.config["CATSHTM_API_URL"] + self.config["CATSHTM_ROUTES"]["crossmatch"],
                           result_format=format,
                           params=params)
    ...
    def catshtm_redshift(self, ra, dec, radius, format="votable", verbose=False):
        ...
        xmatches = self.catshtm_crossmatch(ra=ra, dec=dec, radius=radius,  catalog_name="all")
